...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	try {
		if (!empty($_POST)) {
			...
			$resultado = Registro::deuCerto($stmt);
...
<body class="flex w-screen h-screen">
	<div class="bg-white shadow-lg rounded-lg p-8 w-full">
		...
		<div class="sucesso_box bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative hidden">
			<p><?php if (isset($sucesso)) {
					echo $sucesso;
				}; ?></p>
		</div>

		<form action="#" method="POST" id="cadastro" class="grid grid-cols-1 md:grid-cols-2 gap-6">
...
<script>
	...
	// 	e.preventDefault();
	// 	const sucesso_box = document.querySelector('.sucesso_box');
	// 	const resultado = <?php $resultado; ?>;
	// 	alert(resultado);
	// 	if (resultado) {
	// 		sucesso_box.classList.remove('hidden');
	// 	};
	// })
</script>
